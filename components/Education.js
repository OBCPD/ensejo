...
const Education = props =>{
    ...
    for( let i in props.tags){
        minorities += (i == 0)? props.tags[i] : props.tags[i].toLowerCase();
        minorities += (i != props.tags.length - 1)?', ' : '';
    }
    return (
        <View style = {styles.container} onPress = {props.onTouch}>
                ...
                <View style = {styles.textContainer}>
                        ...
                        <Text style = {styles.label}>School/College name: {props.name}</Text>
                        <Text style = {styles.label}>Destined to: {minorities}</Text>
