...
const ManifestationScreen = props => {
    ...
    const renderI = dataI =>{
        ...
        return(
            <Manifestation name = {dataI.item.title} link = {dataI.item.link} desc = {dataI.item.description} />
        );
    }
    useEffect(()=>{
      
        fetch('https://my-test-esejo-app-default-rtdb.firebaseio.com/registers/Manifestations.json').then((response) => response.json()).then((responseJson) =>{
...
ManifestationScreen.navigationOptions = navData => {
    return{
        headerTitle: 'Manifestations',
    };
};
