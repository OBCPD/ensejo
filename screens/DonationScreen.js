...
const DonationScreen = props => {
    ...
    const renderI = dataI =>{
        ...
        return(
            <Donation name = {dataI.item.title} link = {dataI.item.link} desc = {dataI.item.description} />
        );
    }
    useEffect(()=>{
      
        fetch('https://my-test-esejo-app-default-rtdb.firebaseio.com/registers/Donations.json').then((response) => response.json()).then((responseJson) =>{
